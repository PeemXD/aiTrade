...
import (
	...
	"github.com/local/polymarket-fetch-data-service/pkg/kafka"
...
func OpenKafkaProducer(cfg config.Config, log *slog.Logger, serviceName string) (*kafka.Producer, error) {
	...
	return kafka.NewProducer(kafka.NewConfig(cfg.KafkaEnabled, cfg.KafkaBrokers, cfg.KafkaClientID, cfg.KafkaConsumerGroupFetch, cfg.KafkaAutoCreateTopics), serviceName, log)
}
